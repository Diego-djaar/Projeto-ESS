Feature: Historico de Pedidos
  ...
  Scenario: Obter pedidos por CPF
    ...
    When uma requisição GET é enviada para "/backend/api/Orders/orders_by_user/111.222.333-44"
  ...
  Scenario: Usuário quer visualizar um pedido específico
    ...
    When é solicitado uma requisição GET para "/backend/api/Orders/order_from_user/" com o parâmetro CPF do usuário "111.222.333-44" e ID de pedido "1" 
    Then o status de resposta JSON deve ser "200"
   ...
   Scenario: Obter pedidos com determinado Filtro
     ...
     When uma requisição POST é enviada para "/backend/api/Orders/orders_filtered/" com o body contendo o CPF "111.222.333-44", o price_max "20" e o restante dos campos nulos
     Then o status de resposta JSON deve ser "200"
