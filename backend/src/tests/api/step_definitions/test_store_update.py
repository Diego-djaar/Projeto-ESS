...
def remove_test_from_db():
    ...
    store_database.add_store(Store.new("37.555.457/0001-90", "Hortifruti@loja.com", "htft", "alimento", "Horti"))
...
def login_request(request_type: str, store_retrieve_pass: str, cnpj: str , nsenha: str, email: str, client=TESTCLIENT):
    retrieval_request = {
        "cnpj": cnpj,
        "email": email,
        "nsenha": nsenha
...
@then(parsers.cfparse('senha da loja "{nome}" agora é "{senha}"'))
def check_password(nome: str, senha: str,  context):
    assert store_database.get_store_by_name(nome)["senha"] == senha
